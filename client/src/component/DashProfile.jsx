...
const DashProfile = () => {
  ...
  const updateUserInfo = async (data) => {
    if (authUser.username === data.username && authUser.email === data.email) {
  ...
  return (
    <div className="max-w-lg flex flex-col  items-center w-full mx-auto">
      ...
      <input type="file" ref={filePickerRef} onChange={handleImage} />
      <form
        onSubmit={handleSubmit(updateUserInfo)}
        className="w-full flex items-center flex-col gap-4"
      >
        <div
          className="size-32 shadow-md cursor-pointer overflow-hidden"
          onClick={() => filePickerRef.current.click()}
        >
          <img
            src={imageFileUrl || authUser.profilePicture}
            alt="user"
